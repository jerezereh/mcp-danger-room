...
import { CharacterDataToICard, CrisisDataToICard } from '../../dataTypes/transforms';
...
import characterData from '../../../assets/CharacterCards.json';
import crisisData from '../../../assets/CrisisCards.json';

function rosterReducer(roster: ICharacterCardProps[], action: any) {
  try {
...
export function RosterView(_props: {}) {
  ...
  function onRowClick(event: BaseSyntheticEvent) {
    const result = characterData.Characters.find(c => c.Name === event.target.parentNode.childNodes[0].outerText);
    if (result === undefined) {
      return;
    }
  ...
  function saveRoster() {
    ...
    getState().forEach((item: ICharacterCardProps) => Characters.push(item.name));
  ...
  return (
    <>
      <Container style={{ flexDirection: 'row' }}>
        ...
        <CardTable
          dataSet={characterData.Characters.map(CharacterDataToICard)}
          onRowClick={onRowClick}
          onRowDoubleClick={addToRoster}
        />
        <InfoPane card={selectedCard} />
        <RosterTable dataSet={roster} onRowClick={onRowClick} onRowDoubleClick={removeFromRoster} />
        <div>
          <AppBar position="sticky">
